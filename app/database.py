...
def get_row_count(cursor):
    ...
    try:
        ...
        app_logger.send_log_to_seq(f"Ошибка при работе с PostgreSQL: {err}")
        raise
...
def execute_sql(query, data_for_query=None, db_name=None):
    ...
    app_logger.send_log_to_seq(f"PostgreSQL - - Выполняется запрос.",
...
def execute_sql_list_as_transaction(queries, db_name=None):
    ...
    try:
        ...
        app_logger.send_log_to_seq("PostgreSQL - - Начало транзакции.")
